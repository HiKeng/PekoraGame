...
Shader "AmplifyShaderPack/URP/SubstanceMix"
{
	Properties
	{
		[HideInInspector] _AlphaCutoff("Alpha Cutoff ", Range(0, 1)) = 0.5
		[HideInInspector] _EmissionColor("Emission Color", Color) = (1,1,1,1)
		[ASEBegin]_Snow_BricksbaseColor("Snow_Bricks - baseColor", 2D) = "white" {}
		_Snow_Bricksheight("Snow_Bricks - height", 2D) = "white" {}
		_Snow_Bricksnormal("Snow_Bricks - normal", 2D) = "white" {}
		[ASEEnd]_Displacement("Displacement", Range( 0 , 2)) = 0
	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 _Snow_Bricksheight_ST;
			float4 _Snow_BricksbaseColor_ST;
			float4 _Snow_Bricksnormal_ST;
			float _Displacement;
			...
			#endif
			CBUFFER_END
			sampler2D _Snow_Bricksheight;
			sampler2D _Snow_BricksbaseColor;
			sampler2D _Snow_Bricksnormal;
			...
			VertexOutput VertexFunction( VertexInput v  )
			{
				...
				float2 uv_Snow_Bricksheight = v.texcoord.xy * _Snow_Bricksheight_ST.xy + _Snow_Bricksheight_ST.zw;
				...
				#endif
				float3 vertexValue = ( tex2Dlod( _Snow_Bricksheight, float4( uv_Snow_Bricksheight, 0, 0.0) ) * _Displacement * float4( v.ase_normal , 0.0 ) ).rgb;
				#ifdef ASE_ABSOLUTE_VERTEX_POS
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_Snow_BricksbaseColor = IN.ase_texcoord7.xy * _Snow_BricksbaseColor_ST.xy + _Snow_BricksbaseColor_ST.zw;
				
				float2 uv_Snow_Bricksnormal = IN.ase_texcoord7.xy * _Snow_Bricksnormal_ST.xy + _Snow_Bricksnormal_ST.zw;
				
				float3 Albedo = tex2D( _Snow_BricksbaseColor, uv_Snow_BricksbaseColor ).rgb;
				float3 Normal = UnpackNormalScale( tex2D( _Snow_Bricksnormal, uv_Snow_Bricksnormal ), 1.0f );
				float3 Emission = 0;
				float3 Specular = 0.5;
				float Metallic = 0;
		...
		Pass
		{
			...
			VertexOutput VertexFunction( VertexInput v )
			{
				...
				float2 uv_Snow_Bricksheight = v.ase_texcoord.xy * _Snow_Bricksheight_ST.xy + _Snow_Bricksheight_ST.zw;
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_Snow_BricksbaseColor = IN.ase_texcoord2.xy * _Snow_BricksbaseColor_ST.xy + _Snow_BricksbaseColor_ST.zw;
